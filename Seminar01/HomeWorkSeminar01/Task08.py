# Задача 8: Требуется определить, можно ли от шоколадки 
# размером n × m долек отломить k долек, 
# если разрешается сделать один разлом по прямой между дольками 
# (то есть разломить шоколадку на два прямоугольника).
# *Пример:*
# 3 2 4 -> yes
# 3 2 1 -> no

n = int(input('Введите количество долек по высоте: '))
m = int(input('Введите количество долек по ширине: '))              
k = int(input('Сколько долек Вы хотите отломить? '))
if k >= n * m:
    print('С такими условиями задачку не решить, попробуйте еще раз')
elif k % n == 0 or k % m == 0:
    print('Молодец, но для шоколадки уже поздно! Ложись спать!')
else: print("Так ломать нельзя, попробуй еще раз")
